...
class MusicBot(discord.Client):
    ...
    async def update_now_playing_message(self, player, entry, server=None, *, channel=None):
        ...
        if player.current_entry:
            ...
            if last_np_msg:
                self.server_specific_data[channel.server]['last_np_msg'] = await self.safe_edit_message(last_np_msg, new='Current %s:' % ('stream' if stream else 'song'), embed=newembed, send_if_fail=True)
            else:
                self.server_specific_data[channel.server]['last_np_msg'] = await self.safe_send_message(channel, content='Current %s:' % ('stream' if stream else 'song'), embed=newembed)
...
#######################################################################################################################
    ...
    async def safe_send_message(self, dest, content, embed=None, **kwargs):
    ...
    async def safe_edit_message(self, message, new, *, embed=None, send_if_fail=False, quiet=False):
        lfunc = log.debug if quiet else log.warning
    ...
    async def cmd_p(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        return self.cmd_player(player, channel, author, permission, leftover_args, song_url)
    ...
    async def cmd_np(self, player, channel, server, message):
        ...
        if player.current_entry:
            ...
            if last_np_msg:
                self.server_specific_data[server]['last_np_msg'] = await self.safe_edit_message(last_np_msg, new='Current song:', embed=last_np_embed)
            else:
                self.server_specific_data[server]['last_np_msg'] = await self.safe_send_message(channel, content='Current song:', embed=last_np_embed)
    ...
    async def on_message(self, message):
        ...
        if not handler:
            await self.safe_send_message(message.channel, 'Unknown command, type %shelp for more info' % self.config.command_prefix, expire_in=10, also_delete=message)
            return
